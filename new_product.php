...
<style>
  ...
  <label class="" for="customSwitch1">Toggle this switch element</label>
    ...
    <div class="col-md-2">
        <button class="btn btn-primary" type="button" id="new_product">Add</button>
    </div>
</div>
      ...
      <div class="col-md-6">
        <button class="btn btn-primary" form="manage-product">Save</button>
         ...
         <button class="btn btn-secondary" href="./index.php?page=product_list">Cancel</button>
      </div>
...
<script>
  ...
  $('#new_product').click(function(){
	...
	$('#manage-product').submit(function(e){
		e.preventDefault()
		start_load()
    if($('#product-items tbody tr').length <= 0){
      alert_toast("Please add atleast 1 product information.","error")
      end_load()
      return false;
    }
		$.ajax({
        ...
        if(resp == 1){
            ...
            setTimeout(function(){
              location.href = 'index.php?page=product_list';
            },2000)
  ...
  function calc(){
        ...
        var total = 0 ;
         $('#product-items [name="price[]"]').each(function(){
          var p = $(this).val();
              p =  p.replace(/,/g,'')
              p = p > 0 ? p : 0;
