...
Class Action {
		...
		foreach($price as $k => $v){
			...
			if(empty($id)){
				...
				while($i == 0){
					...
					$chk = $this->db->query("SELECT * FROM products where reference_number = '$ref'")->num_rows;
					if($chk <= 0){
						$i = 1;
					}
				}
				$data .= ", reference_number='$ref' ";
				if($save[] = $this->db->query("INSERT INTO products set $data"))
					$ids[]= $this->db->insert_id;
			}else{
				if($save[] = $this->db->query("UPDATE products set $data where id = $id"))
					$ids[] = $id;
			}
		}
	...
	function update_product(){
		extract($_POST);
		$update = $this->db->query("UPDATE products set status= $status where id = $id");
		$save = $this->db->query("INSERT INTO product_tracks set status= $status , product_id = $id");
		if($update && $save)
			return 1;  
	}
	function get_product_heistory(){
		extract($_POST);
		$data = array();
		$product = $this->db->query("SELECT * FROM products where reference_number = '$ref_no'");
		if($product->num_rows <=0){
			return 2;
		}else{
			$product = $product->fetch_array();
			$data[] = array('status'=>'Item accepted by Courier','date_created'=>date("M d, Y h:i A",strtotime($product['date_created'])));
			$history = $this->db->query("SELECT * FROM product_tracks where product_id = {$product['id']}");
	...
	function get_report(){
		extract($_POST);
		$data = array();
		$get = $this->db->query("SELECT * FROM products where date(date_created) BETWEEN '$date_from' and '$date_to' ".($status != 'all' ? " and status = $status " : "")." order by unix_timestamp(date_created) asc");
		$status_arr = array("Item Accepted by Courier","Collected","Shipped","In-Transit","Arrived At Destination","Out for Delivery","Ready to Pickup","Delivered","Picked-up","Unsuccessfull Delivery Attempt");
